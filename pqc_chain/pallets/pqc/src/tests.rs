...
fn register_keys_twice_fails() {
	new_test_ext().execute_with(|| {
		let kp = MlDsaKeypair::generate();
		assert_ok!(Pqc::register_keys(RuntimeOrigin::signed(1), kp.public, None));
		assert_noop!(
			Pqc::register_keys(RuntimeOrigin::signed(1), kp.public, None),
...
fn verify_signature_works() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(Pqc::verify_signature(RuntimeOrigin::signed(2), 1, msg, sig));
	});
}
...
fn remove_keys_works() {
	new_test_ext().execute_with(|| {
		let kp = MlDsaKeypair::generate();
		assert_ok!(Pqc::register_keys(RuntimeOrigin::signed(1), kp.public, None));
		assert_ok!(Pqc::remove_keys(RuntimeOrigin::signed(1)));
		assert!(!PqcKeys::<Test>::contains_key(1));
	});
}
...
fn register_validator_keys_works() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(Pqc::register_validator_keys(
			RuntimeOrigin::signed(1),
			kp.public,
			Some(kem.public)
		));

		assert!(Pqc::validator_keys_of(&1).is_some());
	});
}
